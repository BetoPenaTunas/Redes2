 ...
 * @author axele
 */
public class ServDescargas {
    
    /*
        ---> PARA LAS DESCARGAS, MI Servidor actuará como un cliente y el Cliente como un 
        servidor <---
    */
    public  Socket cl;
    public void envioDeCliente(){
        ...
        String rutaDeDescarga= carpetaRaiz.getAbsolutePath()+"\\Nube";
        try{
            
            int pto = 8007;
            String dir = "localhost";   // Podemos poner local HOST DIR==IP PUBLICA  
            cl = null;    // Se intentará conectar al servidor que está en el PUERTO 8007
            cl=this.conectaSocket();
            System.out.println("Conexion con servidor establecida, elija sus archivos a descargar");
            JFileChooser jf = new JFileChooser(rutaDeDescarga);   // Creando mi instancia del FileChooser, por default podemos ELEGIR SÓLO UN ARCHIVO
...
//              Debido a que si mandamos archivos grandes, no tenemos tanta memoria 
    ...
    public Socket conectaSocket() throws IOException{
            ...
            int pto = 8007;
            String dir = "localhost";   // Podemos poner local HOST DIR==IP PUBLICA  
            Socket c = new Socket(dir,pto);    // Se intentará conectar al servidor que está en el PUERTO 8007
            return c;
    }
