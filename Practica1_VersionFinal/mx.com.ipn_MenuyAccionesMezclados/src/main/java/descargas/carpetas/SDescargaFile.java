 ...
 * @author axele
 */
public class SDescargaFile {
    public Socket cl;
    public static ServerSocket s;
    public void recibeServidor() {
        try {
            int pto = 8009;   // App corriendo en este puerto
            s = new ServerSocket(pto);
            s.setReuseAddress(true);  // Si se cierra el socket, generamos uno nuevo

            
            System.out.println("Servidor iniciado esperando solicitud de descarga de carpetas..");
...
//                No importa si mis paquetes se DESORDENAN ya que TCP los ordena
    ...
    public String creaDirectorio(DataInputStream dis) throws IOException {
        ...
        String ruta = f.getAbsolutePath() + "\\Descargas";    // Obtenga el PATH del archivo, que en este momento es LA CARPETA RAÍZ 
...
//              Obtengo el flujo de escritura de BYTES, (LO QUE SALE)
                DataOutputStream dos = new DataOutputStream(cl.getOutputStream());
                cl.shutdownInput();
//              Flujo de lectura DE BYTES para leer desde el SISTEMA DE ARCHIVOS, habrá uno del lado del SDescargaFile.java
